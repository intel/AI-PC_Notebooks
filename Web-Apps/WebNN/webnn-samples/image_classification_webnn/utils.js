...
export function getUrlParams() {
  ...
  if (numRuns < 1) {
    console.log(`Ignore the url param: 'numRuns', its value must be >= 1.`);
    numRuns = 1;
  }
  ...
  if (powerPreference && !powerPreferences.includes(powerPreference)) {
    console.log(`Ignore the url param: 'powerPreference', its value must be ` +
  ...
  if (numThreads != null) {
    ...
    if (!Number.isInteger(numThreads) || numThreads < 0) {
      console.log(`Ignore the url param: 'numThreads', its value must be ` +
